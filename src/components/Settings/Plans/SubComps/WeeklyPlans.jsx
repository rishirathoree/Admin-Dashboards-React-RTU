...
const WeeklyPlans = () => {
    ...
    const reservations = []
    for (let i = 0; i < 7; i++) {
        reservations.push({ randomMailSubjects: 'jbsvcdsvh',randomMailSubjects: Math.floor(Math.random() * 100), })
    }

    return (
        <div className='my-4 mx-2 rounded py-4 px-4 space-y-4'>
            <p className='font-medium text-[12px]'>Weekly Plans</p>
            <div className=" rounded-lg border-gray-200 border-[1px] overflow-x-auto">
                <table className="border-collapse table-auto w-full text-sm">
                    <thead>
                        <tr className="">
                            <th className="font-semibold min-w-min py-2 px-4 text-[11px] text-left">Days</th>
